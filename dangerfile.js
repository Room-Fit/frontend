...
if (!/^#\d+/.test(danger.github.pr.title)) {
    dangerTestResults.IS_PR_INCLUDE_ISSUE_NUMBER.value = false;
}

// 2. Reviewer 지정
if (danger.github.requested_reviewers.users.length === 0) {
    dangerTestResults.IS_REVIEWER_ASSIGNED.value = false;
}

// 3. Assignee 지정
if (!danger.github.pr.assignee) {
    dangerTestResults.IS_LABEL_ASSIGNED.value = false;
}
...
schedule(async () => {
    try {
        ...
        dangerTestResults.IS_TYPESCRIPT_COMPILE_SUCCESS.value = false;
    }
});
...
schedule(async () => {
    try {
        ...
        dangerTestResults.IS_ESLINT_SUCCESS.value = false;
    }
});
